...
async function tryRefreshClients() {
  setNotification('Refreshing clients list...', 'info');
  try {
    const result = await window.electronAPI.refreshClients();
    if (result.success) {
      setNotification('Clients list updated!', 'success');
    } else {
      setNotification('Update failed, using cached data.', 'error');
    }
  } catch (e) {
    setNotification('Update failed, using cached data.', 'error');
  }
...
function renderClientList() {
  console.log('Rendering client list. Favorites:', Array.from(favorites));
...
function setNotification(msg, type = 'info') {
  ...
  if (type === 'info') {
    notif.style.color = '#ffd60a';
  } else if (type === 'success') {
    notif.style.color = '#2ecc40';
  } else if (type === 'error') {
    notif.style.color = '#ff4d4f';
  } else {
    notif.style.color = '#ffd60a';
  }
  notif.style.display = 'block';
  // Align to right edge of search field and top of refresh button
  if (searchInput && refreshBtn) {
    const searchRect = searchInput.getBoundingClientRect();
    const refreshRect = refreshBtn.getBoundingClientRect();
    notif.style.right = (window.innerWidth - searchRect.right + 16) + 'px';
    notif.style.top = refreshRect.top + 'px';
  } else {
    notif.style.right = '16px';
    notif.style.top = '36px';
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (refreshBtn) {
    refreshBtn.addEventListener('click', async () => {
      ...
      await tryRefreshClients();
      loadFavorites(); // Always reload favorites after refresh
      await loadClients();
      renderClientList();
    });
